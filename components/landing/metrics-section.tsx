...
const metrics = [
  { 
    ...
    suffix: "+", 
    prefix: "",
    label: "Du khách mỗi năm",
    sublabel: "đến tham quan Ô Quy Hồ",
  },
  { 
    value: 365, 
    suffix: "", 
    prefix: "",
    label: "Ngày đẹp",
    sublabel: "quanh năm có thể tham quan",
  },
  { 
    value: 15, 
    suffix: "°C", 
    ...
    sublabel: "khí hậu mát mẻ quanh năm",
  },
];
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-32">
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              <span className="flex items-center gap-2 px-3 py-1 bg-[#4ade80]/10 text-[#4ade80] text-xs">
                <span className="w-2 h-2 rounded-full bg-[#4ade80] animate-pulse" />
                THỜI TIẾT TỐT
              </span>
              <span className="text-sm text-muted-foreground">
                {time ? `${time.toLocaleTimeString("vi-VN")} - Việt Nam` : ""}
              </span>
            </div>

            <h2 className={`text-5xl md:text-6xl lg:text-[110px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              Thống kê
              <br />
              <span className="text-muted-foreground">du lịch.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 mt-12">
          ...
          <div className={`lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
            ...
            <div className="mb-6">
              <DotGraph color="green" height={36} freq1={0.28} freq2={0.09} freqT={0.5} speed={0.018} baseline={0.35} amplitude={0.55} />
            </div>
            <div className="text-lg text-foreground mb-2">{metrics[0].label}</div>
            <div className="text-sm text-muted-foreground">{metrics[0].sublabel}</div>
          </div>
          ...
          {metrics.slice(1).map((metric, index) => (
            ...
            >
              <div className="w-full">
                <div className="text-sm text-muted-foreground mb-2">{metric.sublabel}</div>
        ...
        {/* Bottom ticker */}
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm text-muted-foreground transition-all duration-1000 delay-500 ${
          isVisible ? "opacity-100" : "opacity-0"
        }`}>
          <span>Mùa xuân</span>
          <span>Mùa hạ</span>
          <span>Mùa thu</span>
          <span>Mùa đông</span>
          <span className="text-foreground">Đẹp quanh năm</span>
        </div>
      </div>
    </section>
  );
